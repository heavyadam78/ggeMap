...
function extractKingdomData(allPlayers, kingdomKey) {
    const kingdomPlayers = [];
    allPlayers.forEach((player) => {
        const kData = player.kingdoms && player.kingdoms[kingdomKey];
        if (kData && (kData.castle || kData.ruin)) {
            kingdomPlayers.push({
                name: player.name,
                rank: player.rank,
                ...kData,
